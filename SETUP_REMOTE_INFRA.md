...
```bash
curl -i -X POST http://13.250.36.84:3000/auth/login \
  -H 'Content-Type: application/json' \
  -d '{"username":"demo","password":"demo123"}'
...
1. Cập nhật mọi placeholder `http://<YOUR_EXTERNAL_IP_OR_DOMAIN>` trong `kong/kong.yml` thành `http://13.250.36.84` (hoặc tên miền của bạn) và đảm bảo đúng cổng.
2. Triển khai lại Kong (`docker compose -f docker-compose.kong-only.yml up -d --build` hoặc `docker compose restart kong`).
3. Trong container Kong chạy `curl http://13.250.36.84:3000/health` để kiểm tra khả năng truy cập usersvc.
...
- Các plugin `pre-function` (validation), `jwt`, `rate-limiting`, `http-log` hoạt động và log hiển thị trên Kibana (`http://13.250.36.84:5601`).
- Script k6 chạy trên máy gateway với `MODE=base UPSTREAM_HOST=http://13.250.36.84:3000` và `MODE=gw GATEWAY_HOST=http://<GATEWAY_IP>:8000` để so sánh trước/sau.
